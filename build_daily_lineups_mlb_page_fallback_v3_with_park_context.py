#!/usr/bin/env python3
"""
build_daily_lineups_mlb_page_fallback_v3_with_park_context.py
...
This revision adds park-context columns so the downstream HR model can apply
park factors by VENUE / HOME TEAM instead of guessing from the batting team.
"""
...
def normalize_team_abbr(value: Optional[str]) -> str:
...
def get_projected_team_lineup(
    ...
    sub = projected_df[
        ...
        (projected_df["team"] == normalize_team_abbr(team)) &
        (projected_df["opponent"] == normalize_team_abbr(opponent))
    ].copy()
...
def game_record_to_rows(game: dict, target_date: str, projected_df: pd.DataFrame, debug: bool = True) -> List[Dict]:
    ...
    venue_blob = game.get("venue") or {}
    venue_name = str(
        venue_blob.get("name")
        or ((venue_blob.get("defaultCoordinates") or {}).get("name"))
        or ""
    ).strip()
...
def build_daily_lineups(target_date: str, projected_path: str = "projected_lineups.csv", debug: bool = True) -> pd.DataFrame:
    ...
    if not frame.empty:
        frame = frame.drop_duplicates().sort_values(by=["game_pk", "team", "batter_name", "pitcher_name"]).reset_index(drop=True)
    return frame


def parse_args(args=None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build daily_lineups.csv using live feed, MLB preview/page lineups, roster fallback, projected fallback, and park-context columns."
    )
